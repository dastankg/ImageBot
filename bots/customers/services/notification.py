...
async def send_monthly_notification(bot):
    ...
    owner_telephones = await sync_to_async(lambda: list(Telephone.objects.filter(is_owner=True)))()

    for telephone in owner_telephones:
        try:
            if telephone.chat_id:
                await bot.send_message(chat_id=telephone.chat_id, text=message, reply_markup=keyboard)
